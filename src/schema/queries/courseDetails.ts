...
import connection from '../types/connections/courseDetailConnection';
...
export default {
  ...
  resolve: async (_, args: QueryArguments, context: Context) => {
    ...
    const data = await context.repositories.courseDetail.find({
      first: args.first,
      after,
      query: args.query,
    });
    const count = await context.repositories.courseDetail.count({
      query: args.query,
    });
    const edges = nodesToEdges(data, after);
