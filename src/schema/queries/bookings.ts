...
import connection from '../types/connections/bookingConnection';
...
export default {
  ...
  resolve: async (_, args: QueryArguments, context: Context) => {
    ...
    const data = await context.repositories.booking.find({
      first: args.first,
      after,
      query: args.query,
    });
    const count = await context.repositories.booking.count({
      query: args.query,
    });
