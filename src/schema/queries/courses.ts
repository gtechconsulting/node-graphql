...
import connection from '../types/connections/courseConnection';
...
export default {
  ...
  resolve: async (_, args: QueryArguments, context: Context) => {
    ...
    const data = await context.repositories.course.find({
      first: args.first,
      after,
      query: args.query,
    });
    const count = await context.repositories.course.count({
      query: args.query,
    });
